...
public class PlayerDeserializer extends JsonDeserializer<Player>{
    ...
    @Override
    public Player deserialize(JsonParser p, DeserializationContext ctx) throws IOException, JacksonException{
        Player result = null;

        try{
            String activePlayerUsername = p.getText();
            result = this.playerService.findByUsername(activePlayerUsername);
