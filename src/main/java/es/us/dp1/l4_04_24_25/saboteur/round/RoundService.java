...
public class RoundService {
    ...
    public Round findByGameIdAndRoundNumber(Integer gameId, Integer roundNumber) {
        return roundRepository.findByGameIdAndRoundNumber(gameId, roundNumber)
                .orElseThrow(() -> new ResourceNotFoundException("Round", "GameId and RoundNumber",
                        gameId + " and " + roundNumber));
    }
    ...
    @Transactional(readOnly = true)
    public Round findByBoardId(Integer boardId) {
        return roundRepository.findByBoardId(boardId)
                .orElseThrow(() -> new ResourceNotFoundException("Round", "BoardId", boardId));
    }
    ...
    public Round initializeRound(Game game, Integer roundNumber) {
        ...
        int leftCards = 70 - (numPlayers * cardsPerPlayer);

        Round round = standardRoundBuilder
                .withGame(game)
                .withRoundNumber(roundNumber)
                .withLeftCards(leftCards)
